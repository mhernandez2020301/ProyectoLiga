...
function asignarEquipo(golFavor, golContra, idEquipo, idUsuario) {
    ...
    Equipo.findOneAndUpdate({ _id: idEquipo, idUsuario: idUsuario }, { $inc: { golesLocal: golFavor, golesVisita: golContra, puntos: punto, partidosJugados: 1 } }, { new: true }, (err, equipoActualizado) => {
...
function crearPartido(req, res) {
    ...
    if (req.params.liga == null) return res.status(500).send({ mensaje: "Debe poner el nombre de la liga a la que quiere asignar el partido" });
